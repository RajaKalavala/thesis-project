...
> Companion docs: [THESIS_UNDERSTANDING.md](../THESIS_UNDERSTANDING.md) · [golden-data/](../golden-data/) (the curated 65-row RAGAS-ready subset built from the dev split) · [../../plan.md](../../plan.md)
...
There is **no** explicit `id` field in the upstream files. The golden dataset (see `docs/golden-data/`) generates its own `golden_id` and `question_id` (e.g. `dev_7`) when constructing reference rows.
...
- The **golden-set construction** (`docs/golden-data/methodology.md` §1) deliberately oversamples long vignettes to **20%** of its 100-question seed. Don't conflate the two rates — the golden set is *not* a representative sample of MedQA.
...
| Median | 34 |
| Used by | golden-set retrieval query construction (`question + answer + metamap_phrases` joined into one search string per `methodology.md` §3) |
...
Consequence:
- The pilot golden-set retrieval (Notebook 04) found that 33.9% of all top-10 retrieved candidates came from Harrison's — *higher* than its 25% share — because Harrison's covers a broad range of clinical topics that match many question types (`docs/golden-data/analysis.md` §7).
...
- **Sentence boundaries are unreliable** — many "sentences" are bullet fragments or table rows. A naive sentence-splitter will produce a long tail of 1–3-word "sentences." This is why the golden-set methodology uses a **recursive 200-token chunker with 20-token overlap** rather than sentence-based chunking.
...
This is roughly aligned with the USMLE Step 1 + Step 2&3 question pool — supports the assumption that retrieval can find answers for almost any MedQA question if the chunking + retrieval are good enough. The pilot golden-set construction confirmed this: 99 / 100 questions had `is_evidence_sufficient = True` in Pass 1 (`docs/golden-data/analysis.md` §3).
...
> **Legacy 65-row golden set in [`golden-data/`](../../golden-data/):** built earlier with MiniLM + 200-token chunks. **Not consumed by the from-scratch plan** — the new BGE-large + 400/80-token chunks would invalidate every `chunk_id` reference in it. Kept as reference material only; see `docs/golden-data/methodology.md` for the construction methodology being re-used.
