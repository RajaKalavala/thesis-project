...
- [x] Notebook `04b_exp02_naive_rag.ipynb` — three gated stages (smoke 50 → golden 234 → **test 1,273**), reuses the §3 runner with `NaiveRetriever` swapped in. Includes split-stratified accuracy breakdown + retrieval spot-check cells. (2026-05-06 — built; Stage C surface narrowed to test split per the 2026-05-06 lock)
- [x] Notebook `04b_exp02_ragas.ipynb` — three gated stages (smoke 10 → full 234 → NaN rescore), runs **all 5 RAGAS metrics** against `golden_234/predictions.jsonl`. Resilience layer (`RunConfig(max_workers=4, ...)`) is on by default; expected on-first-pass NaN rate <5 %. (2026-05-06 — built; user runs manually)
- [x] Smoke 50 ran (`Acuuracy = 0.78`) and golden 234 ran (`Acuuracy = 0.85`) on 2026-05-06; Stage C *partial* full_12723 run was started + stopped at 4,844/12,723 rows, then **abandoned** under the 2026-05-06 evaluation-surface lock (the 4,844 partial rows were 100 % train, no test rows touched, so zero cache utility for the new test-split surface).
- [ ] Run baseline notebook Stage C (test 1,273) — user-driven, ~10 min, $0
- [ ] Outputs to `results/exp_02_naive_rag__{smoke_50,golden_234,test_1273}/` (canonical = `test_1273`)
- [ ] Run RAGAS notebook Stage A (smoke 10 ~ $0.50) → Stage B (full 234 ~ $11–12) → Stage C if needed
- [ ] Metrics: Exact Match Accuracy + RAGAS suite (5 metrics on golden 234) + Retrieval Recall@K + latency
- **Tables touched:** Table 1 (row 2), Table 8 (row 1), Table 9

### EXP_03 — Sparse RAG (BM25)

- [ ] Notebook `04c_exp03_sparse_rag.ipynb`
- [ ] Run on **test 1,273** with the BM25 index (per [plan.md §0 #8](../plan.md))
- **Tables touched:** Table 1, Table 8, Table 9

### EXP_04 — Hybrid RAG (RRF)

- [ ] Notebook `04d_exp04_hybrid_rag.ipynb`
- [ ] Run on **test 1,273** — BGE + BM25 fused with RRF k=60 (per [plan.md §0 #8](../plan.md))
- **Tables touched:** Table 1, Table 8, Table 9

### EXP_05 — Multi-Hop RAG

- [ ] Notebook `04e_exp05_multi_hop_rag.ipynb`
- [ ] Run on **test 1,273** — BGE-large dense retrieval per hop (per [plan.md §0 #8](../plan.md))
