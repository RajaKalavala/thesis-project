...
### EXP_03 — Sparse RAG (BM25) ✅ BASELINE COMPLETE (2026-05-07) · RAGAS pending (batch with EXP_04/05)
...
- [x] Notebook `04c_exp03_ragas.ipynb` — three gated stages, all 5 RAGAS metrics. Inspect cell auto-compares Context Precision / Faithfulness vs EXP_02. (2026-05-07 — built; user is **batching RAGAS** until all 4 RAG baselines done)
- [x] **Canonical: `exp_03_sparse_rag__test_1273/`** — `Acuuracy = 0.7581` (965 / 1,273), mean_latency = 0.435 s, wall_time = **97 min** (BM25 retrieval is ~40× slower per query than ChromaDB HNSW because `rank-bm25` has no inverted index — every query is O(N=67k chunks) in pure Python; cost still $0). **−1.57 pp vs EXP_01 No-RAG**, **+0.08 pp vs EXP_02 Naive Dense** (essentially tied with dense).
- [x] Outputs to `results/exp_03_sparse_rag__{smoke_50,golden_234,test_1273}/`
- [x] **Two empirical findings to anchor EXP_04 Hybrid hypothesis** (full analysis in [`output_notes/04c_exp03_output.md`](output_notes/04c_exp03_output.md)):
  - **Complementarity**: 153 of 1,273 test questions disagree between dense & sparse, with 50/50 right/wrong split (76 dense-right vs 77 sparse-right) → strong signal that RRF fusion should help.
  - **USMLE-step pattern**: sparse beats dense on step2&3 (+1.68 pp), dense beats sparse on step1 (+1.33 pp) → orthogonal strengths along clinical-decision vs basic-recall axis.
- [ ] **RAGAS run pending** — will run after EXP_04 + EXP_05 baselines complete, then judge all 4 RAG architectures as a batch (cost ~$36–48 across the 4)
- **Sharpened hypothesis for EXP_04**: *Hybrid Acuuracy on test_1273 should land **above 0.76** (above both EXP_02 and EXP_03). Falsified if Hybrid ≤ 0.76.*
- **Tables touched:** Table 1 (row 3 — Acuuracy filled, RAGAS pending), Table 8 (row 2 — pending RAGAS), Table 9 (Sparse-RAG column)

### EXP_04 — Hybrid RAG (RRF)

- [x] Notebook `04d_exp04_hybrid_rag.ipynb` — three gated stages (smoke 50 → golden 234 → test 1,273), uses refactored `HybridRetriever` (BGE + ChromaDB + BM25 fused via RRF k=60). Inspect cell tests the falsifiable hypothesis (Hybrid Acuuracy > 0.76). (2026-05-07 — built; user runs manually)
- [x] Notebook `04d_exp04_ragas.ipynb` — three gated stages, all 5 RAGAS metrics. Inspect cell auto-compares Context Precision vs EXP_02's 0.33 to test the *Hybrid CP ≥ 0.50* hypothesis. (2026-05-07 — built; user runs manually)
- [ ] Run baseline notebook (Stage A → B → C) — user-driven, ~30–60 min total, $0
- [ ] Run RAGAS notebook (Stage A smoke ~$0.50 → Stage B full ~$11–12 → Stage C if needed) — **user is batching all 4 RAG architectures' RAGAS together**
- [ ] Outputs to `results/exp_04_hybrid_rag__{smoke_50,golden_234,test_1273}/` (canonical = `test_1273`)
- **Falsifiable hypotheses** (anchored in [output_notes/04c_exp03_output.md §4](output_notes/04c_exp03_output.md)): (1) Hybrid Acuuracy on test_1273 > 0.76 (above both EXP_02 and EXP_03); (2) Hybrid Context Precision ≥ 0.50 (above EXP_02's 0.33).
- **Tables touched:** Table 1 (row 4), Table 8 (row 3), Table 9 (Hybrid-RAG column)

### EXP_05 — Multi-Hop RAG

- [x] Notebook `04e_exp05_multi_hop_rag.ipynb` — three gated stages, uses new `MultiHopRetriever` (3-hop iterative dense, sub-query generation via Groq, dedup across hops, early stop on no-progress, returns up to 15 chunks per question). Inspect cell tests the falsifiable hypothesis. (2026-05-07 — built; user runs manually)
- [x] Notebook `04e_exp05_ragas.ipynb` — three gated stages, all 5 RAGAS metrics. Inspect cell extracts the multi-hop golden subset (`requires_multihop=yes`, n=13) and tests whether Multi-Hop Faithfulness > 0.05 on rows where Naive scored 0.000. (2026-05-07 — built; user runs manually)
- [ ] Run baseline notebook (Stage A → B → C) — user-driven, ~30–45 min total, $0 (3× more Groq calls per question vs EXP_02–04 due to sub-query generation)
- [ ] Run RAGAS notebook (Stage A smoke ~$0.50 → Stage B full ~$13–15 — slightly higher than EXP_02–04 due to bigger context per question, up to 15 chunks vs 5)
- [ ] Outputs to `results/exp_05_multi_hop_rag__{smoke_50,golden_234,test_1273}/`
- [x] Hop budget = 3, k=5 per hop, terminate on no-new-chunks (per [plan.md §0 #7](../plan.md), implemented in `src/retrieval/multi_hop.py`)
- **Falsifiable hypotheses** (anchored in [output_notes/04b_exp02_output.md §3.5](output_notes/04b_exp02_output.md)): (1) Multi-Hop Acuuracy on test_1273 ≥ EXP_02's 0.7573 (else doesn't earn its 3× compute cost); (2) Multi-Hop Faithfulness on the 13 multi-hop golden rows > 0.05 (vs EXP_02's 0.000).
- **Tables touched:** Table 1 (row 5), Table 8 (row 4), Table 9 (Multi-Hop column)
...
- [ ] Notebook `05_exp07_adaptive_rag.ipynb` — **test 1,273** (per [plan.md §0 #8](../plan.md))
