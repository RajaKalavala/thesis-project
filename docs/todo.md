...
- [x] Notebook `04a_exp01_base_llm.ipynb` — three gated stages: smoke 50 → golden 234 → full 12,723. (2026-05-05)
- [x] Run Stage A (smoke 50, **Acuuracy = 0.940**), Stage B (golden 234, **0.902**), Stage C (full 12,723, **0.869**) (2026-05-05)
- [x] Outputs to `results/exp_01_base_llm__{smoke_50,golden_234,full_12723}/` (one dir per surface) — `predictions.jsonl` + `retrieval.jsonl` + `summary.json` for each (2026-05-05)
- [x] **Methodology finding (contamination empirically validated):** train + dev = 0.880, **test = 0.774** ⇒ 10.6 pp gap. Aligns with literature LLaMA-class No-RAG ceiling (~75–78 %) on the test split; confirms [plan.md §15](../plan.md) contamination risk. Discussion-chapter implication: report all subsequent architectures with paired (full / test-split) numbers and pivot the headline narrative toward hallucination control via RAGAS Faithfulness + the confidence-aware rejection layer. See [output_notes/04a_exp01_output.md](output_notes/04a_exp01_output.md).
