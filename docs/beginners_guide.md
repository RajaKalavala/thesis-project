...
**What you'll do.** Take **300** of the 12,723 questions (sized for budget-efficiency — see plan.md §0 #9). For each one, ask `gpt-4o` (locked 2026-05-04 after empirical A/B against the open-weights `gpt-oss-120b` alternative — see [docs/output_notes/04_notebook_output.md](output_notes/04_notebook_output.md)) to:
...
**Smoke test (CRITICAL).** Build **50 questions first** before the rest of the 300. Costs ~$1.10 with `gpt-4o`. The pilot is the **quality gate** — if accept rate, sufficiency, or multi-hop calibration look off, tune the prompts before scaling.

**Cost / time (measured 2026-05-04).** **$6.61** for the full 300-question build ($1.08 pilot + $5.53 production scale-up) + ~80 min wall-clock. The M1 Pro just orchestrates.
...
1. **Right now:** make sure your `.env` has all **3 API keys** (locked 2026-05-04): `GROQ_API_KEY` (LLaMA answerer), `OPENAI_API_KEY` (gpt-4o golden-set constructor), `ANTHROPIC_API_KEY` (Claude RAGAS judge). Confirm with a 3-call smoke test.
