...
|---|---|---|---|
| 1 | **Naive RAG** | Single-step dense semantic retrieval (ChromaDB top-k, BGE-large embeddings) | Baseline. Question → embed → vector search → top-k → prompt → LLM. |
| 2 | **Sparse RAG** | BM25 keyword retrieval only | Tokenize question → BM25 score against textbook chunks → top-k → prompt → LLM. |
| 3 | **Hybrid RAG** | Dense (ChromaDB) + Sparse (BM25) fused with **Reciprocal Rank Fusion** (k=60) | Two parallel retrieval paths merged by RRF, then LLM. |
...
## 6. Shared Stack (locked — held constant across all 4 architectures)
...
### MedQA USMLE
- 12,723 English multiple-choice questions (USMLE-style) = train 10,178 + dev 1,272 + test 1,273.
- Source: Jin et al. (2020) — `arxiv.org/abs/2009.13081`.
- License: CC-BY-4.0, no PHI.
- Two parallel files in the upstream distribution: 5-option (A–E, canonical) and 4-option (A–D, with `metamap_phrases` attached). **Use the 4-option variant** for all experiments — see [docs/dataset/README.md](dataset/README.md) for the field-level reference.
- Schema (4-option variant — what the experiments consume):

| Field | Description | Example | Type |
|---|---|---|---|
| `question` | Clinical scenario / stem | `"A 45-year-old male presents with chest pain..."` | string |
| `options` | Letter → option text | `{"A": "...", "B": "...", "C": "...", "D": "..."}` | dict |
| `answer` | Verbatim correct option text | `"Myocardial infarction"` | string |
| `answer_idx` | Letter of the correct option | `"B"` | string |
| `meta_info` | USMLE step bucket | `"step1"` or `"step2&3"` | string |
| `metamap_phrases` | Pre-extracted clinical concepts | `["chest pain", "45 year old", ...]` | list[str] |
...
MedQA JSONL ──► parse ──► dedupe ──► validate options ──► standardize encoding ──┐
                                                                                  ├─► recursive 400-token chunking ──► BGE-large embedding ──┬─► ChromaDB collection
...
```
 ┌─────────────┐   ┌──────────────────┐   ┌───────────┐   ┌────────────┐   ┌─────────────────────┐
 │ MedQA       │──►│ Data Cleaning    │──►│ 400-token │──►│ Embedding  │──►│ ChromaDB (dense)    │
 │ (12,723 Q)  │   │ & Preprocessing  │   │ chunking  │   │ (BGE-large)│   │ + BM25 (sparse)     │
 │ + Textbooks │   │                  │   │           │   │            │   │                     │
 └─────────────┘   └──────────────────┘   └───────────┘   └────────────┘   └──────────┬──────────┘
                                                                                     │
                          ┌──────────────────────────────────────────────────────────┤
...
### In-scope
- Implementation & evaluation of the four baseline RAG architectures **plus** three novelty layers (Adaptive Routing, Confidence-Aware Rejection, Hallucination Error-Type Taxonomy) — see [plan.md](../plan.md) for the full 16-experiment programme.
- MedQA US (English, **12,723 MCQs** — train + dev + test combined) as the evaluation surface.
- ChromaDB for dense, BM25 for sparse retrieval.
- Five RAGAS metrics + LIME/SHAP explainability + multi-signal confidence rejection.
...
- Comparison with non-RAG baselines beyond the included `EXP_01 No-RAG` baseline (no prompt-only or fine-tuned variants).
...
1. **Shared infrastructure** — one set of preprocessing, chunking, embedding, ChromaDB collection, BM25 index, Groq client, prompt template, RAGAS evaluator, LIME/SHAP analyzers — used by all four architectures (and by the three novelty layers).
...
3. **One evaluation harness** — runs all four architectures over the full 12,723 MedQA questions, captures answers + retrieved passages + RAGAS scores + explainability outputs into a structured results store.
...
The cleanest starting point is the shared infrastructure (data → chunks → BGE embeddings → ChromaDB + BM25 indices), then Naive RAG end-to-end as the baseline, then layer in the other three architectures one at a time. See [plan.md](../plan.md) for the full step-by-step build order.
